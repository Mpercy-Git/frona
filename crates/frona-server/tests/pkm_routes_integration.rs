...
fn pkm_router() -> axum::Router<AppState> {
    frona::api::routes::memory_pkm::router()
        .merge(frona::api::routes::memory_pkm_read::router())
}
...
async fn reset_accepts_an_ordinary_user_and_clears_memory_in_the_background() {
    ...
    tokio::time::timeout(std::time::Duration::from_secs(2), async {
        while repo.entity_by_path("u1", "projects/reset-me").await.unwrap().is_some() {
            tokio::task::yield_now().await;
        }
    })
    .await
    .expect("reset worker did not finish");
    assert!(repo.entity_by_path("u1", "projects/reset-me").await.unwrap().is_none());
...
async fn config_returns_forbidden_without_memory_scope() {
    ...
    let app = pkm_router().with_state(state);
    let resp = app.oneshot(get("/api/memory/pkm/sync/config", &token)).await.unwrap();
...
async fn config_get_and_set_round_trip() {
    ...
    let resp = app.clone().oneshot(get("/api/memory/pkm/sync/config", &token)).await.unwrap();
    ...
    let body = axum::body::to_bytes(resp.into_body(), usize::MAX).await.unwrap();
    let json: serde_json::Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(json["memoryDirectory"], "Brain", "directory updated via POST");
}
...
async fn manifest_empty_for_fresh_user() {
    ...
    let token = pat(&state, vec!["memory".into()]).await;
    let app = pkm_router().with_state(state);
    let resp = app.oneshot(get("/api/memory/pkm/sync/manifest", &token)).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    let body = axum::body::to_bytes(resp.into_body(), usize::MAX).await.unwrap();
    let json: serde_json::Value = serde_json::from_slice(&body).unwrap();
...
async fn graph_returns_the_user_scoped_memory_network() {
    ...
    .await
    .unwrap();
    seed_asserted_entity_link(&state.db, "u1", "people/me", "projects/frona", "schema:creator")
        .await
        .unwrap();
    ...
    let me = json["nodes"].as_array().unwrap().iter()
...
async fn graph_adds_shared_memory_edges_only_without_a_direct_relation() {
    use frona::memory::pkm::model::{MemoryKind, EntityCategory};

    let (state, _tmp) = test_state(MemoryBackend::Pkm).await;
    let token = pat(&state, vec!["memory".into()]).await;
    let repo = frona::db::repo::pkm::PkmRepo::new(state.db.clone(), 20);
    for (path, name) in [("people/a", "A"), ("people/b", "B"), ("people/c", "C")] {
        repo.upsert_entity_skeleton("u1", path, EntityCategory::Concept,
            &[], name, "Person", &[]).await.unwrap();
    }
    repo.create_sourced_memory(
        "u1", MemoryKind::Fact, "A and B share a fact.",
        &["people/a".into(), "people/b".into()], vec![],
    ).await.unwrap();
    repo.create_sourced_memory(
        "u1", MemoryKind::Fact, "B and C share a fact.",
        &["people/b".into(), "people/c".into()], vec![],
    ).await.unwrap();
    seed_asserted_entity_link(&state.db, "u1", "people/b", "people/c", "schema:knows")
        .await.unwrap();

    let app = pkm_router().with_state(state);
    let resp = app.oneshot(get("/api/memory/pkm/graph", &token)).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    let body = axum::body::to_bytes(resp.into_body(), usize::MAX).await.unwrap();
    let json: serde_json::Value = serde_json::from_slice(&body).unwrap();
    let edges = json["edges"].as_array().unwrap();
    assert_eq!(edges.len(), 2);
    let shared = edges.iter().find(|edge| edge["origin"] == "memory").unwrap();
    ...
    assert!(edges.iter().any(|edge| {
        edge["origin"] == "asserted" && edge["fromPath"] == "people/b" && edge["toPath"] == "people/c"
    }));
}

#[tokio::test]
async fn page_exposes_article_structure_relations_and_memory_evidence() {
    use frona::memory::pkm::model::{EvidenceSource, EvidenceStrength, MemoryEvidence, MemoryKind, EntityCategory};

    let (state, _tmp) = test_state(MemoryBackend::Pkm).await;
    let token = pat(&state, vec!["memory".into()]).await;
    let repo = frona::db::repo::pkm::PkmRepo::new(state.db.clone(), 20);
    repo.upsert_entity_skeleton("u1", "people/me", EntityCategory::Concept,
        &["https://schema.org/Person".into()], "Test", "Owner", &[]).await.unwrap();
    repo.upsert_entity_skeleton("u1", "projects/frona", EntityCategory::Concept,
        &["https://schema.org/Project".into()], "Frona", "Memory system", &[]).await.unwrap();
    ...
    seed_asserted_entity_link(&state.db, "u1", "people/me", "projects/frona", "schema:creator")
        .await.unwrap();
    repo.create_sourced_memory(
        "u1", MemoryKind::Fact, "Test builds Frona.", &["people/me".into()],
        vec![MemoryEvidence {
            strength: EvidenceStrength::Explicit,
            source: EvidenceSource::UserMessage {
                message_id: "m1".into(), chat_id: "c1".into(), quote: "I build Frona".into(),
            },
        }],
    ).await.unwrap();

    let app = pkm_router().with_state(state);
    let resp = app.oneshot(get("/api/memory/pkm/entity?path=people%2Fme", &token)).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    let body = axum::body::to_bytes(resp.into_body(), usize::MAX).await.unwrap();
    let json: serde_json::Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(json["entity"]["body"], "# Test\n\nBuilds [[projects/frona]].");
    ...
    assert_eq!(json["memories"][0]["evidence"][0]["source"]["user_message"]["message_id"], "m1");
}
...
async fn search_uses_the_existing_ranked_entity_search() {
    ...
    let repo = frona::db::repo::pkm::PkmRepo::new(state.db.clone(), 20);
    repo.upsert_entity_skeleton(
        "u1", "projects/frona", frona::memory::pkm::model::EntityCategory::Concept,
        &["https://schema.org/Project".into()], "Frona", "A private personal memory system", &[],
    ).await.unwrap();

    let app = pkm_router().with_state(state);
    let resp = app.oneshot(get("/api/memory/pkm/search?q=personal", &token)).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    let body = axum::body::to_bytes(resp.into_body(), usize::MAX).await.unwrap();
    let json: serde_json::Value = serde_json::from_slice(&body).unwrap();
