    ...
    use crate::memory::pkm::consolidation::reconcile::projection::{
        ...
        property_replacement_rejections, unsupported_scope_relations,
    };
    ...
    use crate::memory::pkm::consolidation::reconcile::{
        AttributeSourceInput, EntityRelation, EntityRetraction, EntityVerdict, Outdated,
        Related, RelationInput,
    };
    use crate::memory::pkm::consolidation::comparison_key;
    use crate::memory::pkm::model::{
        AttributeSource, EntityCategory, KnowledgeEntity, KnowledgeEntityLink,
        Episode, EpisodeStatus, KnowledgeMemory, RelationType, TemporalAnchor,
    };
    use crate::memory::pkm::model::{Disposition, EvidenceSource, EvidenceStrength, MemoryEvidence, MemoryKind};
    ...
    fn reconcile_mint_requires_intent_and_does_not_infer_an_inverse_from_one_pair() {
        ...
        assert!(!reconcile_declaration_rejections(
            &without_declaration, &HashSet::new(), &HashSet::new(), &px()).is_empty());
        ...
        assert!(reconcile_declaration_rejections(
            &with_intent, &HashSet::new(), &HashSet::new(), &px()).is_empty());
        assert!(!with_intent.declarations[0].edits().iter().any(|edit| {
            matches!(edit, crate::memory::pkm::ontology::SchemaEdit::InverseProperties { .. })
        }));
    }
    ...
    fn object_property_cannot_be_reused_as_a_literal_attribute() {
        ...
        let rejections = reconcile_declaration_rejections(
            &verdict, &known, &HashSet::new(), &px(),
        );

        assert!(
            rejections.iter().any(|rejection| rejection.contains("object property")),
    ...
    fn data_property_cannot_be_reused_as_an_entity_relation() {
        ...
        let rejections = reconcile_declaration_rejections(
            &verdict, &HashSet::new(), &known, &px(),
        );

        assert!(
            rejections.iter().any(|rejection| rejection.contains("data property")),
    ...
    fn reconcile_projects_completed_task_plans_as_history() {
        ...
        assert_eq!(current.iter().map(|memory| memory.id.as_str()).collect::<Vec<_>>(),
            ["sun-completed", "mon-completed"]);
        assert_eq!(historical.iter().map(|memory| memory.id.as_str()).collect::<Vec<_>>(),
...
fn promotion_suggestions_consider_pending_peer_entities() {
        let pending = KnowledgeEntity {
            id: String::new(), user_id: "u".into(), path: "services/quote-feed".into(),
            origin: crate::memory::pkm::model::EntityOrigin::Internal,
            category: EntityCategory::Concept, kinds: vec!["schema:Service".into()],
            name: "Quote Feed".into(), description: String::new(), identity_evidence: Vec::new(),
            attribute_sources: Vec::new(), source_memory_ids: Vec::new(), body: String::new(),
            sync_content: None,
            mirrored_rev: None,
            extracted_rev: None,
            related_playbooks: Vec::new(), search_text: "Quote Feed".into(),
            search_names: Vec::new(), search_name_tokens: Vec::new(), search_assertions: Vec::new(),
            attributes: serde_json::json!({}), use_count: 0,
            aliases: ["Market API".to_string()].into_iter().collect(), rev: None,
            updated_at: Utc::now(), rendered_at: chrono::DateTime::<Utc>::MIN_UTC,
        };
        let verdict: EntityVerdict = serde_json::from_value(serde_json::json!({
            "attributes": {"provider": "Market API"}
        })).unwrap();
        let suggestions = promotion_suggestions(
            "projects/dashboard", &verdict, &[pending], &[],
        );
...
fn global_relation_between_different_entity_scopes_is_rejected() {
        ...
        let scopes = HashMap::from([
            (
                ...
                vec!["assistant/example-assistant".to_string(), "people/me".to_string()],
            ),
            (
...
fn global_relation_with_identical_entity_scope_is_allowed() {
        ...
        assert!(unsupported_scope_relations(&verdict, &scopes, "assistant/example-assistant").is_empty());
    }
...
fn replacement_may_have_broader_scope_when_both_memories_share_the_current_entity() {
        ...
        let scopes = HashMap::from([
            ("former-employer".to_string(), vec!["organizations/former-corp".to_string(), "people/me".to_string()]),
            ("current-employer".to_string(), vec![
    ...
    fn object_property_replacement_retires_all_old_source_memories() {
        ...
        assert!(property_replacement_rejections(
            &verdict,
            &memories,
            &[],
            &[old_link],
        ...
        let replacements = verdict.relations.iter()
            .flat_map(|related| related.links.iter().map(move |link| {
                (related.memory.clone(), link.to.clone())
            }))
            .collect::<HashSet<_>>();
    ...
    fn object_property_rejection_explains_source_set_difference() {
        ...
        let rejected = property_replacement_rejections(
            &verdict, &memories, &[], &[old_link],
        );

        assert_eq!(rejected.len(), 1, "{rejected:?}");
        assert!(rejected[0].contains("expected [\"old-fact\"]"), "{rejected:?}");
        assert!(rejected[0].contains("supplied [\"old-broad\", \"old-fact\"]"), "{rejected:?}");
        assert!(rejected[0].contains("unexpected [\"old-broad\"]"), "{rejected:?}");
    }
    ...
    fn named_and_quotable_change_is_accepted() {
        let memories = [mem("old", "postgres runs on port 5432"), mem("new", "postgres runs on port 5433")];
        assert!(replace_is_supported(&replace("5432", "5433"), "old", "new", &memories));
    }
    ...
    fn identical_restatement_is_refused() {
        ...
        for link in [replace("", ""), replace(same, same), replace("Rust", "Rust")] {
            assert!(
    ...
    fn quoting_is_matched_leniently() {
        let memories = [mem("old", "Port is 5432"), mem("new", "port  is   5433")];
        assert!(replace_is_supported(&replace("port IS 5432", "Port is 5433"), "old", "new", &memories));
    }

    #[test]
    fn outdated_memory_names_the_edge_target_it_retires() {
        let content = comparison_key("employer: Former Corp");
        assert!(memory_names_entity(&content, "Former Corp", &HashSet::new()));
    ...
    fn rephrase_still_passes_the_text_checks() {
        let memories =
            [mem("old", "the cache uses Redis."), mem("new", "the cache runs on a Redis instance.")];
    ...
    fn unsupported_replaces_reports_the_pairs_it_refused() {
        ...
        let verdict = EntityVerdict {
            relations: vec![
                Related { memory: "old".into(), links: vec![replace("same", "same")] },
        ...
        assert!(rejected[0].contains("old") && rejected[0].contains("new"), "{rejected:?}");
    }

    #[test]
    fn retiring_a_memory_requires_every_orphaned_assertion_to_change() {
        let memories = [mem("old", "employer: Former Corp"), mem("new", "employer: Example Corp")];
        ...
        let stale = EntityVerdict {
            ...
            relation_retractions: Vec::new(),
            entity_relation_replacements: Vec::new(),
            outdated: vec![Outdated { memory: "old".into(), note: "joined Example Corp".into() }],
        ...
        assert!(rejected.iter().any(|line| line.contains("frona:employer")), "{rejected:?}");
        assert!(rejected.iter().any(|line| line.contains("relation_retractions")), "{rejected:?}");
    ...
    fn multi_entity_memory_cannot_source_a_data_attribute() {
        let memories = [mem("relationship", "Casey Owner named the assistant Example Assistant")];
        let verdict = EntityVerdict {
            relations: Vec::new(),
            entity_relations: Vec::new(),
            relation_retractions: Vec::new(),
        ...
        let rejected = assertion_provenance_rejections(
            &verdict, &memories, &[], &[], &scopes, "people/me",
        );

        assert_eq!(rejected.len(), 1, "{rejected:?}");
        assert!(rejected[0].contains("multi-entity"), "{rejected:?}");
        assert!(rejected[0].contains("assistants/example-assistant"), "{rejected:?}");
    }
    ...
    fn already_camel_case_key_keeps_its_case() {
        ...
        assert_eq!(curie_key("firmware releaseDate", &px()), "frona:firmwareReleaseDate");
    }
    ...
    fn acronym_is_lowered_rather_than_kept() {
        assert_eq!(curie_key("AI assistant name", &px()), "frona:aiAssistantName");
    ...
    fn key_with_a_colon_is_only_kept_when_it_is_a_usable_term() {
        ...
        assert!(m.contains_key("frona:firmwareVersion"), "a usable CURIE is kept: {m:?}");
        assert!(
        ...
        assert!(!m.contains_key("dc:title"), "an unbound prefix is not a CURIE: {m:?}");
        assert!(m.contains_key("frona:title"), "`dc:title` keeps its local name: {m:?}");
        for k in m.keys() {
    ...
    fn curie_keys_map_standard_mint_frona_and_keep_curies() {
        ...
        assert!(!m.contains_key("email") && !m.contains_key("port"), "originals removed: {m:?}");
    }
    ...
    fn attribute_already_held_as_a_relation_is_dropped() {
        ...
        let out = drop_keys_held_as_relations(attrs, &[link("schema:worksFor")], &HashSet::new(), &px());
        let m = out.as_object().unwrap();
        assert!(!m.contains_key("schema:worksFor"), "the promoted fact stays an edge: {m:?}");
    ...
    fn relation_check_compares_expanded_iris_not_spellings() {
        ...
        let out =
            drop_keys_held_as_relations(attrs, &[link("https://schema.org/worksFor")], &HashSet::new(), &px());
        assert!(
    ...
    fn unrelated_relations_drop_nothing() {
        ...
        let out = drop_keys_held_as_relations(attrs.clone(), &[link("schema:knows")], &HashSet::new(), &px());
        assert_eq!(out, attrs);
    }
