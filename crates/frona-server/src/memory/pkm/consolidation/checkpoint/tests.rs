    ...
    fn only_the_stages_that_need_state_carry_it() {
        ...
        assert_eq!(
            stateful,
            ["ingest", "classify", "resolve", "reconcile", "assemble", "playbook_resolve"]
        );
    }
    ...
    fn pending(
        ...
        crate::memory::pkm::model::KnowledgeConsolidationEntity::pending(
            "test-run", "user", "topics/example",
            crate::memory::pkm::model::EntityCategory::Concept,
            vec![contribution], [memory_id.to_string()].into_iter().collect(),
        )
    }
    ...
    fn staged_attributes_expose_conflicting_values_from_every_contribution() {
        ...
        assert_eq!(former.staged_attributes(), serde_json::json!({
