...
use crate::memory::pkm::model::EvidenceStrength;
    ...
    fn ambiguous_agent_quote_feedback_lists_candidate_source_messages() {
        ...
        let sources = vec![
            transcript_source("m1", "Compute Box has 64 GB.", TranscriptEvidenceKind::AgentMessage {
                message_id:"a1".into(), agent_id:"agent".into(), chat_id:"chat".into(),
            }),
            transcript_source("m2", "Compute Box has 64 GB.", TranscriptEvidenceKind::AgentMessage {
                message_id:"a2".into(), agent_id:"agent".into(), chat_id:"chat".into(),
            }),
            transcript_source("m3", "The comparison is complete.", TranscriptEvidenceKind::AgentMessage {
                message_id:"a3".into(), agent_id:"agent".into(), chat_id:"chat".into(),
            }),
        ];

        let failure = validate_batch(&batch, &sources).into_iter()
            .find(|failure| failure.reason == "quote_not_found").unwrap();
    ...
    fn structural_grounding_failures_name_the_broken_invariant() {
        ...
        let user_sources = vec![transcript_source(
            ...
            TranscriptEvidenceKind::UserMessage {
                message_id: "message-1".into(), chat_id: "chat-1".into(),
            },
        ), transcript_source(
            "m2",
            T,
            TranscriptEvidenceKind::UserMessage {
                message_id: "message-2".into(), chat_id: "chat-1".into(),
            },
        )];
        for (memory, expected) in cases {
            ...
            assert!(failures.iter().any(|failure| failure.reason == expected),
        ...
        assert!(validate_batch(&unknown_playbook, &sources(T)).iter()
    ...
    fn procedural_memory_may_cite_multiple_passages_from_one_agent_message() {
        ...
        assert!(!failures.iter().any(|failure| failure.reason == "procedural_requires_one_assertion_source"),
    ...
    fn confirmation_failures_distinguish_missing_claim_from_ineligible_source() {
        let user_sources = vec![transcript_source(
            "m1", "Yes, Postgres runs on 5433.",
            TranscriptEvidenceKind::UserMessage {
                message_id: "message-1".into(), chat_id: "chat-1".into(),
            },
        )];
        let confirmation = SourceCitation {
            message: "m1".into(), quote: "Postgres runs on 5433".into(),
            strength: EvidenceStrength::Explicit, confirmation: true,
        };
        let handles = HashSet::from(["m1"]);
        assert_eq!(resolve_citation(&confirmation, &user_sources, &handles).unwrap_err(),
            "confirmation_without_cited_agent_claim");

        let agent_sources = sources(T);
        assert_eq!(resolve_citation(&confirmation, &agent_sources, &handles).unwrap_err(),
    ...
    fn correction_cannot_rewrite_an_unrejected_memory_field() {
        ...
        assert_eq!(merged.memories[0].sources[0].quote, "Postgres is a database");
    }
    ...
    fn correction_may_drop_a_rejected_memory_without_shifting_an_accepted_sibling() {
        ...
        let failures = vec![GroundingFailure {
            field_path:"memories[0]".into(), message:"m1".into(),
            submitted:"Unsupported Agent claim".into(), reason:"agent_claim_needs_tool_evidence",
        }];
