...
use crate::memory::pkm::model::{PLAYBOOK_KIND_IRI, EntityCategory};
...
pub(crate) fn redact_text(input: &str, configured_secrets: &[String]) -> String {
    ...
    for secret in configured_secrets.iter().filter(|secret| !secret.is_empty()) {
...
pub(crate) fn redact_json(
    ...
    match value {
        ...
        serde_json::Value::Array(values) => serde_json::Value::Array(values.iter()
            .map(|value| redact_json(value, configured_secrets)).collect()),
...
fn sensitive_key(key: &str) -> bool {
    ...
    [
        ...
        "accesskey", "privatekey", "clientsecret", "credential", "cookie",
    ].iter().any(|sensitive| key.contains(sensitive))
}
...
impl ReadMemoryContextTool {
    async fn execute(
        ...
        if !consume(&self.total) {
            return Ok(ToolOutput::text("Tool budget exhausted; submit the best resolution now."));
        }
        ...
        for evidence in &memory.evidence {
            let EvidenceSource::AgentMessage { chat_id, message_id, .. } = &evidence.source else {
                continue;
            };
...
impl ReadTranscriptTool {
    async fn execute(
        ...
        if !consume(&self.total) || self.remaining.fetch_update(Ordering::Relaxed, Ordering::Relaxed, |remaining|
        ...
        let tool_calls = self.lookup.tool_calls.get(chat_id).map(Vec::as_slice).unwrap_or_default();
        let limit = arguments.get("limit").and_then(serde_json::Value::as_u64)
            .unwrap_or(20).clamp(1, 20) as usize;
        ...
        for (message_index, message) in messages[start..end].iter().enumerate() {
            ...
            let local = self.lookup.cursors.iter()
                .find_map(|(local, held)| (held == &(chat_id.clone(), absolute)).then_some(local.as_str()))
                .unwrap_or("?");
            let role = match message.role {
                MessageRole::User => "user",
                MessageRole::Agent => "agent",
        ...
        out.push_str(&format!("remaining_calls={}", self.remaining.load(Ordering::Relaxed)));
        Ok(ToolOutput::text(out))
    }
}
...
impl FindPlaybooksTool {
    async fn execute(
        ...
        if !consume(&self.total) {
            return Ok(ToolOutput::text("Tool budget exhausted; submit the best Playbook now."));
        }
        let query = str_arg(&arguments, "query").unwrap_or_default();
        let eligible_paths = self.eligible_playbook_paths.as_deref()
            .map(|paths| paths.iter().cloned().collect());
        let additional_candidates = self.view.as_ref().map(|view| view.entities().iter()
            ...
            .map(|entity| crate::memory::pkm::model::EntityHit {
                ...
                search_assertions: entity.search_assertions.clone(), body: entity.body.clone(),
            }).collect()).unwrap_or_default();
        let subject = Subject::from_parts(
            self.subject_path.clone().unwrap_or_default(), query.to_string(),
            std::iter::empty(), String::new(),
            EntityCategory::Playbook, vec![PLAYBOOK_KIND_IRI.to_string()],
            std::iter::empty(),
        );
        let hits = Search::new(self.repo.clone()).find_candidates(
            Request {
                subject, eligible_paths, additional_candidates,
        ...
        if out.is_empty() { out.push_str("(no Playbooks found)"); }
        Ok(ToolOutput::text(out))
    }
}
...
impl ReadPlaybookTool {
    async fn execute(
        ...
        let Some(entity) = entity.filter(|entity| entity.category == EntityCategory::Playbook) else {
            return Ok(ToolOutput::text("Unknown Playbook path."));
        };
        let path = entity.path.clone();
        self.lookup.read_paths.write().expect("playbook lookup poisoned").insert(path.clone());
        ...
        let source_memories = if memories_by_id.is_empty() {
            ...
            memories_by_id.values().map(memory_bullet).collect::<String>()
        };
        let body = if entity.body.trim().is_empty() { "(not authored yet)" } else { &entity.body };
        Ok(ToolOutput::text(format!(
            "path={}\nname={}\ndescription={}\nsource_memories:\n{}\nbody:\n{}",
            entity.path, entity.name, entity.description, source_memories, redact_text(body, &[])
        )))
    }
}
...
impl GetToolOutputTool {
    async fn execute(
        ...
        if !consume(&self.total) || self.remaining.fetch_update(Ordering::Relaxed, Ordering::Relaxed, |remaining|
            ...
            return Ok(ToolOutput::text("Tool-output budget exhausted; submit the best Playbook now."));
        }
        let local = str_arg(&arguments, "id").unwrap_or_default();
...
fn consume(counter: &AtomicUsize) -> bool {
    counter.fetch_update(Ordering::Relaxed, Ordering::Relaxed, |remaining|
        remaining.checked_sub(1)
    ).is_ok()
}
...
mod tests {
    ...
    fn every_playbook_conversation_tool_has_a_model_definition() {
        ...
        for name in [
            ...
            "read_transcript", "get_tool_output",
        ] {
            let path = format!("tools/pkm/{name}.md");
            assert!(crate::tool::load_tool_definition(&prompts, &path).is_some(),
    ...
    async fn read_playbook_accepts_a_path_supplied_by_the_stage() {
        ...
        let mut pending = crate::memory::pkm::model::KnowledgeConsolidationEntity::pending(
            "test-run", "test-user", "programming/fetch-stock-close-with-yfinance",
        ...
        let view = Arc::new(EntityToolView::new(
            vec![pending.as_knowledge_entity()], usize::MAX, Arc::new(AtomicUsize::new(0)),
        ));
        ...
        let context = InferenceContext::new_detached(
            crate::auth::User {
                id: "test-user".into(), handle: crate::handle!("test-user"),
                email: "test@example.com".into(), name: "Test User".into(),
                password_hash: String::new(), timezone: None, groups: Vec::new(),
                deactivated_at: None, created_at: chrono::Utc::now(), updated_at: chrono::Utc::now(),
            },
            crate::agent::models::Agent {
                id: "test-agent".into(), user_id: "test-user".into(),
                handle: crate::handle!("test-agent"), name: "Test Agent".into(),
                description: String::new(), model_group: "test".into(), enabled: true,
                sandbox_limits: None, max_concurrent_tasks: None, skills: None, avatar: None,
                identity: Default::default(), prompt: None, heartbeat_interval: None,
                next_heartbeat_at: None, heartbeat_chat_id: None,
        ...
        let output = tool.execute(
            ...
            &context,
        ).await.unwrap().text_content().to_string();

        assert!(output.contains("Fetch a stock close price with yfinance"), "{output}");
    ...
    fn find_playbooks_searches_pending_entity_view() {
        ...
        std::thread::spawn(move || {
            ...
            let result = runtime.block_on(async {
                ...
                let mut pending = crate::memory::pkm::model::KnowledgeConsolidationEntity::pending(
                    "test-run", "test-user", "markets/retrieve-stock-close-price",
                    EntityCategory::Playbook, Vec::new(), Default::default(),
                );
                ...
                pending.kinds = vec![PLAYBOOK_KIND_IRI.into()];
                let view = Arc::new(EntityToolView::new(
                    vec![pending.as_knowledge_entity()], usize::MAX,
                    Arc::new(AtomicUsize::new(0)),
                ));
                let tool = FindPlaybooksTool {
                    prompts: prompts(), repo, view: Some(view),
                ...
                let output = tool.execute(
                    "find_playbooks", serde_json::json!({"query":"stock close"}), &context,
                ).await?;
        ...
        let output = receive.recv_timeout(Duration::from_secs(2))
            ...
            .unwrap();
        assert!(output.contains("markets/retrieve-stock-close-price"), "{output}");
    }
}
