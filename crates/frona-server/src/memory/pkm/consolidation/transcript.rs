...
mod tests {
    ...
    fn call(id: &str, message_id: &str, turn: u32, turn_text: Option<&str>) -> ToolCall {
        ToolCall {
            ...
            provider_call_id: format!("provider-{id}"), name: "web_search".into(),
            arguments: serde_json::json!({"query":"firmware"}), result: "result".into(),
            success: true, duration_ms: 1, hitl: None, task_event: None, system_prompt: None,
            description: None, turn_text: turn_text.map(str::to_string), turn_reasoning: None,
    ...
    fn keeps_the_existing_text_shape_when_there_is_no_tool_turn_text() {
        ...
        assert_eq!(message_text("message-1", "  final response  ", &calls), "final response");
    }

    #[test]
    fn keeps_tool_turn_text_when_the_final_response_is_empty() {
        let calls = vec![call("research", "message-1", 0, Some("durable research result"))];

        assert_eq!(message_text("message-1", "  ", &calls), "durable research result");
    }
    ...
    fn external_note_uses_transcript_format() {
        assert_eq!(external_note("firmware note"), "[m1] External note: firmware note");
    }
}
