...
/// One user's materialized graph, kept so repeated reads don't re-reason from scratch.
///
/// The generation counter blocks stale publication: a build that started before an
/// invalidation must not install its result afterwards. The mutex coalesces concurrent
/// builds, so a burst of queries reasons once rather than once each.
#[derive(Default)]
...
impl OntologyManager {
    ...
    /// Reason once per user per graph generation, rather than per query.
