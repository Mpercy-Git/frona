...
pub fn build_abox_triples(
    ...
    for entity in entities {
        ...
        // Every concept carries its name and path as literals. Nothing stores them as
        // attributes, so without these a query cannot ask for a page by name at all.
        ...
        if let Some(map) = entity.attributes.as_object() {
            for (key, val) in map {
                ...
                {
                    // An attribute spelled as one of the built-ins would emit a second,
                    // conflicting triple for the same property.
