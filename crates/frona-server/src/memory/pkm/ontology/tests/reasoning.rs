    ...
    async fn delta_extends_base_and_reasons_across_boundary() {
        ...
        for cls in ["schema:CreativeWork", "schema:Thing"] {
            ...
            assert!(sparql::ask(&reasoned.store, &q, u.prefixes()).unwrap(), "pg inferred {cls}");
        }
    ...
    async fn commit_bumps_version_and_cas_rejects_stale() {
        let (mgr, _repo) = manager().await;
        mgr.commit("u", &[SchemaEdit::DeclareClass { class: "frona:Service".into() }])
            .await
            .unwrap();
        let u = mgr.load("u").await.unwrap();
        assert_eq!(u.version(), 1);
        ...
        let miss = mgr
            .try_commit("u", &[SchemaEdit::DeclareClass { class: "frona:Team".into() }], 0)
            .await
            .unwrap();
        assert!(miss.is_none(), "stale expected_version → CAS miss");

        mgr.commit("u", &[SchemaEdit::DeclareClass { class: "frona:Team".into() }])
            .await
            .unwrap();
        let u = mgr.load("u").await.unwrap();
    ...
    async fn test_edits_distinguishes_consistent_and_incoherent_schema() {
        ...
        let bad = mgr
            .test_edits(
                "u",
                &[
                    SchemaEdit::SubClassOf { sub: "frona:Weird".into(), sup: "schema:Person".into() },
                    SchemaEdit::SubClassOf {
    ...
    async fn edit_validation_sees_pending_edges_and_types_in_a_projected_abox() {
        ...
        let edits = [
            SchemaEdit::DeclareObjectProperty { property: "frona:hasAssistant".into() },
        ...
        let with_projection =
            mgr.test_edits_with_abox("u", &edits, &projected).await.unwrap();
        assert!(
    ...
    async fn sparql_answers_employees_of_org() {
        ...
        let q = format!("SELECT ?p WHERE {{ ?p frona:worksFor <{}> }}", individual_iri("orgs/acme"));
        ...
        people.sort();
        assert_eq!(people, vec!["people/bob".to_string(), "people/sarah".to_string()]);
    }
...
async fn abox_entity_is_queryable_through_the_base() {
    ...
    async fn facet_bound_flags_out_of_range_port() {
        ...
        seed_reconciled_entity(&repo, "u", "services/pg", "", "db", &serde_json::json!({"frona:port": 99999}))
            .await
            .unwrap();
        let violations = mgr.materialize("u").await.unwrap();
        assert!(
        ...
        seed_reconciled_entity(&repo, "u", "services/pg", "", "db", &serde_json::json!({"frona:port": 5432}))
            .await
            .unwrap();
        let violations = mgr.materialize("u").await.unwrap();
        assert!(
            !violations.iter().any(|v| v.source == ViolationSource::Facet),
    ...
    async fn facet_bound_inherited_via_subproperty() {
        ...
        mgr.commit(
            "u",
            &[
                SchemaEdit::DeclareDataProperty { property: "frona:httpPort".into() },
    ...
    async fn restrict_datatype_edit_mints_and_enforces_facet() {
        ...
        seed_reconciled_entity(&repo, "u", "services/pg", "", "db", &serde_json::json!({"frona:replicaCount": 0}))
            .await
            .unwrap();
        let violations = mgr.materialize("u").await.unwrap();
        assert!(
        ...
        seed_reconciled_entity(&repo, "u", "services/pg", "", "db", &serde_json::json!({"frona:replicaCount": 3}))
            .await
            .unwrap();
    ...
    async fn ontology_edit_reports_data_violation_over_real_abox() {
        ...
        let impact = mgr
            ...
            .await
            .unwrap();
        assert!(impact.incoherence.is_empty(), "schema stays coherent: {impact:?}");
        assert!(
            impact.data_violations.iter().any(|v| v.detail.contains("replicaCount")),
