    ...
    async fn projection_is_stored_on_first_load_and_reused() {
        ...
        assert!(!first.effective_ontology().is_empty(), "a typed entity seeds a cut");

        let row = repo.ontology_get("u").await.unwrap().expect("the cut was persisted");
    ...
    async fn stored_projection_reasons_without_the_catalogue() {
        ...
        let reasoned =
            super::super::reasoning::materialize(rebuilt.triples(), &[], &[type_triple(&bob, "https://schema.org/Person")])
                .unwrap();
        let q = format!("ASK {{ <{bob}> a <https://schema.org/Thing> }}");
        assert!(sparql::ask(&reasoned.store, &q, rebuilt.prefixes()).unwrap(), "ancestors survive the round-trip");
    }
    ...
    async fn changed_seed_set_re_cuts_and_an_unchanged_one_does_not() {
        ...
        assert_eq!(same.updated_at, before.updated_at, "no rewrite when nothing moved");
        ...
        assert!(
            !before.seeds.contains(&"https://schema.org/Organization".to_string()),
            "not a seed before"
        );
        assert!(
            after.seeds.contains(&"https://schema.org/Organization".to_string()),
            "is a seed after: {:?}",
            after.seeds
        );
...
async fn deleting_the_last_entity_using_a_term_drops_it_from_the_cut() {
        ...
        let before = repo.ontology_get("u").await.unwrap().unwrap();
        assert!(
            before.effective_ontology.contains("schema.org/SoftwareApplication"),
        ...
        assert!(
            !after.seeds.contains(&"https://schema.org/SoftwareApplication".to_string()),
            "no longer referenced, so no longer a seed"
        );
        assert!(
            !after.effective_ontology.contains("schema.org/SoftwareApplication"),
    ...
    async fn minted_term_does_not_stop_the_cut_from_pruning() {
        let (mgr, repo) = manager().await;
        mgr.commit("u", &[SchemaEdit::DeclareClass { class: "frona:AIAssistant".into() }])
            .await
            .unwrap();
        seed_concept(&repo, "u", "people/bob", "schema:Person").await;
        seed_concept(&repo, "u", "services/pg", "schema:SoftwareApplication").await;
...
async fn removing_a_source_leaves_existing_entities_reasoning() {
        ...
        mgr.install_catalogue().expect("the catalogue still loads without it");
        assert!(
            !mgr.catalogue().unwrap().declares("https://schema.org/Person"),
        ...
        .unwrap();
        let q = format!("ASK {{ <{bob}> a <https://schema.org/Thing> }}");
        assert!(sparql::ask(&reasoned.store, &q, after.prefixes()).unwrap(), "bob is still typed");
    ...
    async fn changed_catalogue_fingerprint_re_cuts() {
        ...
        mgr.load("u").await.unwrap();
        let after = repo.ontology_get("u").await.unwrap().unwrap();
        assert_ne!(after.catalog_fingerprint, "a-different-catalogue", "re-cut and restamped");
    }
