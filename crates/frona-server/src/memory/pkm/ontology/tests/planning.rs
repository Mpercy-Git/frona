...
async fn normalization_keeps_only_the_most_specific_classes() {
        ...
        let got = mgr.normalize_types(
            &[iri("schema:Person"), iri("schema:Thing")],
            &[],
        );
...
async fn normalization_canonicalizes_before_deduplication_and_subsumption() {
        ...
        assert_eq!(
            mgr.normalize_types(
                &["schema:Thing".into(), "schema:Person".into()],
                &[],
            ),
            [iri("schema:Person")],
...
async fn normalization_preserves_order_and_deduplicates() {
        let (mgr, _repo) = manager().await;
        let got = mgr.normalize_types(
            &[iri("schema:CreativeWork"), iri("schema:Person"), iri("schema:CreativeWork")],
            &[],
        );
    ...
    async fn more_specific_type_retires_the_one_it_implies() {
        ...
        let entity = repo.entity_by_path("u", "people/sarah").await.unwrap().unwrap();
        assert_eq!(entity.kinds, ["urn:frona:Engineer"], "Thing and Person are both implied");
    }
...
async fn entity_carries_every_compatible_type() {
        ...
        assert_eq!(
            entity.kinds,
            ["https://schema.org/SoftwareApplication", "urn:frona:Database"]
        );

        let pass = mgr.reason_user("u").await.unwrap();
        let pg = individual_iri("svc/pg");
        for want in ["https://schema.org/SoftwareApplication", "urn:frona:Database"] {
            let q = format!("ASK {{ <{pg}> a <{want}> }}");
            assert!(sparql::ask(&pass.reasoned.store, &q, pass.effective_ontology.prefixes()).unwrap(), "asserted {want}");
        }
        let q = format!("ASK {{ <{pg}> a <https://schema.org/Thing> }}");
        assert!(sparql::ask(&pass.reasoned.store, &q, pass.effective_ontology.prefixes()).unwrap(), "Thing is inferred, not stored");
    }
    ...
    async fn clashing_type_is_rejected_and_the_facts_are_untouched() {
        ...
        seed_reconciled_entity(&repo, "u", "orgs/acme", "", "Acme, the retailer", &serde_json::json!({}))
            .await
            .unwrap();
        let before = repo.entity_by_path("u", "orgs/acme").await.unwrap().unwrap();
        ...
        let after = repo.entity_by_path("u", "orgs/acme").await.unwrap().unwrap();
        assert_eq!(after.kinds, ["https://schema.org/Organization"], "the newest type lost");
        assert_eq!(after.body, before.body, "and the entity's facts are untouched");
    }
    ...
    async fn graph_validation_reports_candidate_witnesses() {
        ...
        entities[0].kinds.push(PrefixMap::standard().expand("schema:Person"));
        let report = mgr.validate_graph("u", &entities, &[], &[], &[]).await.unwrap();
    ...
    async fn graph_validation_admits_terms_from_the_staged_abox() {
        ...
        let report = mgr.validate_graph("u", &entities, &links, &[], &[]).await.unwrap();
        assert!(!report.is_valid(), "the staged predicate's strict domain must be in scope");
        assert!(report.diagnostics.iter().any(|diagnostic| {
            diagnostic.subject.as_deref() == Some("orgs/acme")
        }), "{:#?}", report.diagnostics);

        seed_asserted_entity_link(&repo, 
            "u",
            "orgs/acme",
            "places/office",
        ...
        assert!(after_reload.iter().any(|violation| {
            violation.subject.as_deref() == Some("orgs/acme")
    ...
    async fn schema_includes_remain_advisory_after_catalogue_loading() {
        ...
        let report = mgr.validate_graph("u", &entities, &links, &[], &[]).await.unwrap();
        assert!(report.is_valid(), "schema:domainIncludes must not constrain the A-Box: {:#?}", report.diagnostics);
    }
    ...
    async fn graph_validation_admits_classes_from_the_staged_abox() {
        ...
        seed_concept(&repo, "u", "orgs/acme", "schema:Organization").await;
        let mut entities = repo.list_entities("u").await.unwrap();
        entities[0].kinds.push("http://example.org/strict/Employee".into());

        let report = mgr.validate_graph("u", &entities, &[], &[], &[]).await.unwrap();
        assert!(!report.is_valid(), "the staged class hierarchy must be in scope");
        assert!(report.diagnostics.iter().any(|diagnostic| {
            diagnostic.subject.as_deref() == Some("orgs/acme")
        }), "{:#?}", report.diagnostics);
    }
    ...
    async fn graph_validation_admits_data_properties_from_the_staged_abox() {
        ...
        let report = mgr.validate_graph("u", &entities, &[], &[], &[]).await.unwrap();
        assert!(!report.is_valid(), "the staged data property's strict domain must be in scope");
        assert!(report.diagnostics.iter().any(|diagnostic| {
            diagnostic.subject.as_deref() == Some("orgs/acme")
        }), "{:#?}", report.diagnostics);
    }
    ...
    async fn graph_validation_never_allows_undeclared_custom_usage() {
        ...
        let entities = repo.list_entities("u").await.unwrap();
        let invalid = mgr.validate_graph("u", &entities, &[], &[], &[]).await.unwrap();
        assert!(invalid.diagnostics.iter().any(|diagnostic| {
            matches!(diagnostic.kind, ValidationDiagnosticKind::UndeclaredTerm)
        }));
        let valid = mgr.validate_graph(
            "u", &entities, &[], &[],
            &[SchemaEdit::SubClassOf {
                sub: "frona:Database".into(), sup: "schema:SoftwareApplication".into(),
            }],
        ).await.unwrap();
    ...
    async fn graph_validation_ignores_links_that_do_not_enter_the_abox() {
        ...
        let report = mgr.validate_graph("u", &entities, &links, &[], &[]).await.unwrap();
        assert!(report.is_valid(), "navigation-only links are not ontology assertions: {:#?}", report.diagnostics);
    }
    ...
    async fn re_adding_an_existing_type_changes_nothing() {
        ...
        assert!(stamp(&mgr, &repo, "people/bob", "schema:Person").await, "already held");
        let entity = repo.entity_by_path("u", "people/bob").await.unwrap().unwrap();
...
async fn alignment_normalizes_the_entities_it_moves() {
        ...
        assert_eq!(
            repo.entity_by_path("u", "orgs/acme").await.unwrap().unwrap().kinds,
        ...
        let moved = mgr
            .plan_retype("u", "urn:frona:Company", &iri("schema:Organization"), &planned.triples)
            .await
            .unwrap();
        assert_eq!(moved.len(), 1);
        repo.commit_schema_and_types("u", &planned.owl, DELTA_FORMAT, planned.version, &moved, &[], &[], &[], &[], &[], &[], &[], None)
            .await
            .unwrap();
