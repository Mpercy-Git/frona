...
impl MemorySearch {
    ...
    /// Upstream's structured output. Retained so the two presentations stay in step;
    /// this fork's `memory_search` renders [`ranked_hits`](Self::ranked_hits) instead.
    #[allow(dead_code)]
    pub(crate) async fn execute(
        ...
        let (ranked, semantic, truncated) = self.rank(user_id, query, vault).await?;
    ...
    /// The ranked pages, in the shape this fork's search tool renders: the page body
    /// comes back with them, which is what lets a hit answer a question without a
    /// second `read` call. Upstream's `MemorySearchResult` carries neither body nor
    /// kinds, so it cannot drive that output.
    pub(crate) async fn ranked_hits(
        &self,
        user_id: &str,
        query: &str,
        vault: &VaultScope,
    ) -> Result<Vec<EntityHit>, AppError> {
        let (ranked, _, _) = self.rank(user_id, query, vault).await?;
        Ok(ranked.into_iter().map(Candidate::into_entity_hit).collect())
    }
    ...
    async fn rank(
        ...
        query: &str,
        vault: &VaultScope,
    ) -> Result<(Vec<Candidate>, SemanticSearchResult, bool), AppError> {
        ...
        // Taken rather than consumed: the rest of `semantic` (the class interpretation
        // and any identity ambiguity) is returned to the caller alongside the ranking.
        for candidate in std::mem::take(&mut semantic.candidates) {
...
struct Candidate {
    ...
    /// Kept for this fork's renderer, which tags a concept page with its classes.
    /// Upstream's JSON output drops them, so nothing upstream carries them this far.
    kinds: Vec<String>,
    name: String,
    description: String,
    aliases: Vec<String>,
...
impl Candidate {
    ...
    fn from_semantic(candidate: &SemanticCandidate) -> Self {
        Self {
            ...
            kinds: candidate.entity.kinds.clone(),
    ...
    fn into_entity_hit(self) -> EntityHit {
        EntityHit {
            path: self.path,
            origin: self.origin,
