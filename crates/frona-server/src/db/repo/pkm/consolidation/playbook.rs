...
impl PkmRepo {
    ...
    pub async fn commit_playbook_resolutions(
        ...
        for write in writes {
            ...
            if let Some(entity) = existing {
                ...
                let entity = KnowledgeEntity {
                    ...
                    category: EntityCategory::Playbook,
            ...
            for memory_id in &write.memory_ids {
                ...
                let provisional_paths = write
                    .candidate_paths
                    .iter()
                    .filter(|path| *path != &write.path)
                    .cloned()
                    .collect::<Vec<_>>();
                ...
                if !links.iter().any(|link| link.entity_path == write.path) {
                    let link = KnowledgeEntitySource {
                        ...
                        entity_path: write.path.clone(),
                        created_at: now,
                    };
                    if let Err(error) = tx
                        .query("CREATE type::record('knowledge_entity_source', $id) CONTENT $link")
                    ...
                    .filter(|source| source != &write.path && !provisional_paths.contains(source))
                {
                    ...
                    let edge = KnowledgeEntityLink {
                        id: new_id(),
                        user_id: user_id.to_string(),
                        from_entity_path: source,
                        to_entity_path: write.path.clone(),
                        relation: "playbook".into(),
                        source_memory_ids: vec![memory_id.clone()],
                        origin: LinkOrigin::Asserted,
                        created_at: now,
                    };
                    if let Err(error) = tx
                        .query("CREATE type::record('knowledge_entity_link', $id) CONTENT $edge")
            ...
            let final_page = match final_page {
                ...
                Ok(None) => {
                    let _ = tx.cancel().await;
                    return Err(Self::err(
                        "playbook_resolve_overlay",
                        "projected entity missing",
                    ));
                }
                Err(error) => {
                    let _ = tx.cancel().await;
            ...
            let mut target_row = match target_row {
                ...
                Ok(None) => KnowledgeConsolidationEntity::pending(
                    &completed_checkpoint.consolidation_id,
                    user_id,
                    &write.path,
            ...
            for (candidate_id, source) in write
                .candidate_ids
                .iter()
                .zip(&write.candidate_paths)
                .filter(|(_, path)| *path != &write.path)
            {
        ...
        tx.commit()
            .await
            .map_err(|e| Self::err("playbook_resolve_commit", e))?;
        Ok(())
    }
}
