    ...
    async fn ontology_terms_survive_entities_whose_attributes_are_absent_or_null() {
        ...
        r.upsert_entity_skeleton("u", "svc/pg", EntityCategory::Concept, &[], "PG", "", &[])
            .await
            .unwrap();
        seed_reconciled_entity(&r, "u", "svc/pg", "", "PG", &serde_json::json!({"schema:email": "b@x"}))
            .await
            .unwrap();

        let terms = r.ontology_terms("u").await.expect("a malformed row is not an error");
        assert!(terms.contains(&person), "the typed entity still contributes its class: {terms:?}");
        assert!(
    ...
    async fn concurrent_upserts_of_one_path_converge_on_a_single_entity() {
        ...
        let racers: Vec<_> = (0..32)
            ...
            .collect();
        for t in racers {
            t.await.unwrap().expect("a lost insert race must recover, not error");
        }
        ...
        let n = rows.first().and_then(|v| v.get("count")).and_then(|v| v.as_u64()).unwrap_or(0);
    ...
    async fn re_mention_keeps_everything_a_later_stage_owns() {
        ...
        r.upsert_entity_skeleton(
            "u", "people/sarah", EntityCategory::Concept,
            &[person.clone(), employee.clone()], "Sarah", "Engineer at Acme.", &[],
        )
            .await
            .unwrap();
        ...
        let entity = r.entity_by_path("u", "people/sarah").await.unwrap().unwrap();
        assert_eq!(entity.kinds, [person, employee], "both classes survive an untyped mention");
        assert_eq!(entity.description, "Engineer at Acme.", "reconcile's description is not clobbered");
        assert_eq!(entity.name, "Sarah", "the name is not rewritten by a passing mention");
        assert!(entity.aliases.contains("S"), "but new aliases still union in: {:?}", entity.aliases);
        assert!(
    ...
    async fn only_page_author_marks_an_entity_done() {
        ...
        seed_reconciled_entity(&r, "u", "svc/pg", "", "the db", &serde_json::json!({})).await.unwrap();
        assert_eq!(
            dirty(&r).await,
            ["svc/pg"],
        ...
        assert!(dirty(&r).await.is_empty(), "clean once the article is written");
    }
    ...
    async fn reconcile_renames_an_entity_and_keeps_the_old_name_searchable() {
        ...
        assert!(entity.aliases.is_empty(), "and mints no alias: {:?}", entity.aliases);

        // A real rename: the entries revealed what the abbreviation stood for.
        seed_reconciled_entity(&r, "u", "svc/pg", "PostgreSQL", "the db", &serde_json::json!({}))
            .await
            .unwrap();
        let entity = r.entity_by_path("u", "svc/pg").await.unwrap().unwrap();
        assert_eq!(entity.name, "PostgreSQL");
        assert!(entity.aliases.contains("PG"), "the old title survives: {:?}", entity.aliases);
        assert!(
            entity.search_text.contains("PostgreSQL") && entity.search_text.contains("PG"),
            "findable under both: {}",
            entity.search_text
        );
        assert_eq!(entity.path, "svc/pg", "renaming the title does not move the entity");
        ...
        seed_reconciled_entity(&r, "u", "svc/pg", "PostgreSQL", "the db", &serde_json::json!({}))
            ...
            .unwrap();
        let entity = r.entity_by_path("u", "svc/pg").await.unwrap().unwrap();
        assert_eq!(entity.aliases.len(), 1, "no duplicate alias: {:?}", entity.aliases);
    }
    ...
    async fn retiring_a_memory_re_dirties_the_entities_that_render_it() {
        ...
        let clean_all = |r: &PkmRepo| {
            ...
            async move {
                for p in ["svc/pg", "svc/redis"] {
                    seed_reconciled_entity(&r, "u", p, "", "x", &serde_json::json!({})).await.unwrap();
                    mark_entity_rendered(&r, "u", p).await.unwrap();
                }
                assert!(r.entities_needing_reconciliation("u").await.unwrap().is_empty());
            }
        };
        ...
        for p in ["svc/pg", "svc/redis"] {
            r.upsert_entity_skeleton("u", p, EntityCategory::Concept, &[], p, "", &[]).await.unwrap();
        }
        ...
        clean_all(&r).await;
        r.set_disposition("u", &old, Disposition::Outdated).await.unwrap();
        assert_eq!(r.entities_needing_reconciliation("u").await.unwrap(), ["svc/pg"]);
        ...
        clean_all(&r).await;
        r.set_disposition("u", &new, Disposition::Suspect).await.unwrap();
    ...
    async fn property_stated_about_an_existing_entity_is_merged_in() {
        let r = repo().await;
        r.upsert_entity_skeleton("u", "orgs/example-corp", EntityCategory::Concept, &[], "Example Corp", "a company", &[])
            .await.unwrap();
        ...
        let entity = r.entity_by_path("u", "orgs/example-corp").await.unwrap().unwrap();
        ...
        assert!(r.memories_for_entity("u", "orgs/example-corp").await.unwrap().is_empty());
    }

    #[tokio::test]
    async fn explicit_existing_entity_update_adds_attributes_without_minting_entities() {
        let r = repo().await;
        r.upsert_entity_skeleton("u", "people/me", EntityCategory::Concept, &[], "Casey Owner", "", &[])
            .await.unwrap();
        ...
        let updates = IngestBatch {
            ...
            entity_updates: vec![
                PendingEntityUpdate {
                    path: "people/me".into(),
                    attribute_evidence: attribute_evidence("people/me", &serde_json::json!({ "employer": "Example Corp" })),
                    attributes: serde_json::json!({ "employer": "Example Corp" }),
                },
                PendingEntityUpdate {
                    path: "organizations/missing".into(),
                    attribute_evidence: attribute_evidence("organizations/missing", &serde_json::json!({ "name": "Missing" })),
        ...
        assert_eq!(counts.entities_created, 0, "updates are never entity-creation requests");
        assert!(
            r.entity_by_path("u", "organizations/missing").await.unwrap().is_none(),
        ...
        assert_eq!(
            memories.iter().filter(|memory| memory.content == "employer: Example Corp").count(),
            0,
    ...
    async fn restated_property_is_not_duplicated_or_resurrected() {
        let r = repo().await;
        r.upsert_entity_skeleton(
            "u", "devices/device-x", EntityCategory::Concept, &[], "Device X", "a screwdriver", &[],
        ).await.unwrap();
        let window = IngestBatch {
            entities: vec![PendingEntity {
                ...
                aliases: Vec::new(),
                identity_evidence: Vec::new(),
                attribute_evidence: attribute_evidence("devices/device-x", &serde_json::json!({ "manufacturer": "Example Tools" })),
        ...
        let send = async |b: &IngestBatch| {
            commit_checkpointed_extract_patch(&r, "u", b, None, &[]).await.unwrap()
        };

        send(&window).await;
        // Same statement again: no second copy of the fact, map unchanged.
        send(&window).await;
        let mems = r.memories_for_entity("u", "devices/device-x").await.unwrap();
        assert_eq!(
            mems.iter().filter(|m| m.content == "manufacturer: Example Tools").count(),
            0,
        ...
        seed_reconciled_entity(&r, "u", "devices/device-x", "", "a screwdriver", &serde_json::json!({}))
            .await
            .unwrap();
        seed_asserted_entity_link(&r, "u", "devices/device-x", "orgs/example-tools", "schema:manufacturer")
            .await
            .unwrap();

        // The entity comes up again and the transcript states the same thing.
        send(&window).await;
        let entity = r.entity_by_path("u", "devices/device-x").await.unwrap().unwrap();
        assert_eq!(
            entity.attributes,
            serde_json::json!({}),
