    ...
    async fn playbook_projection_and_checkpoint_transition_commit_together_and_replay() {
        ...
        r.upsert_entity_skeleton(
            "u", "services/postgres", EntityCategory::Concept, &[], "Postgres", "database", &[],
        ).await.unwrap();
        r.upsert_entity_skeleton(
            "u", "playbooks/restart-db", EntityCategory::Playbook, &[],
            "Restart Database", "old scope", &["Database Restart".into()],
        ).await.unwrap();
        let memory = r.create_sourced_memory(
            "u",
        ...
        r.commit_playbook_resolutions("u", &[write.clone()], &checkpoint).await.unwrap();
        ...
        assert!(r.entity_by_path("u", "playbooks/restart-db").await.unwrap().is_none());
        assert_eq!(r.memories_for_entity("u", &write.path).await.unwrap().len(), 1);
        let links = r.links_from_entity("u", "services/postgres").await.unwrap();
        assert!(links.iter().any(|link| link.to_entity_path == write.path && link.relation == "playbook"));
        assert!(matches!(
            r.latest_consolidation_record("u").await.unwrap().unwrap().state,
        ...
        let working = effective.working_entity(&write.path).await.unwrap().unwrap();
        ...
        assert_eq!(r.memories_for_entity("u", &write.path).await.unwrap().len(), 1);
        assert_eq!(
            r.links_from_entity("u", "services/postgres").await.unwrap().into_iter()
                .filter(|link| link.to_entity_path == write.path && link.relation == "playbook").count(),
            1,
    ...
    async fn playbook_resolution_relocates_memory_from_its_provisional_candidate_path() {
        let r = repo().await;
        r.upsert_entity_skeleton(
            "u", "tools/yfinance", EntityCategory::Concept, &[], "yfinance", "library", &[],
        ).await.unwrap();
        r.upsert_entity_skeleton(
            "u", "tools/yfinance/fetch-stock-close-price", EntityCategory::Playbook, &[],
            "Fetch Stock Close Price", "Fetch a close price.", &[],
        ).await.unwrap();
        let memory = r.create_sourced_memory(
            "u", MemoryKind::Procedural, "Diagnose secure Yahoo connectivity",
            &["tools/yfinance".into(), "tools/yfinance/fetch-stock-close-price".into()],
        ...
        let mut pending_candidate = KnowledgeConsolidationEntity::pending(
            &checkpoint.consolidation_id, "u", "tools/yfinance/fetch-stock-close-price",
            EntityCategory::Playbook,
            vec![crate::memory::pkm::PendingEntityContribution {
                name: "Fetch stock close price securely".into(),
                description: "Provisional diagnostic scope.".into(), aliases: Default::default(),
        ...
        r.upsert_consolidation_entity(&pending_candidate).await.unwrap();
        let write = PlaybookResolutionWrite {
            candidate_ids: vec![candidate_id],
            candidate_paths: vec!["tools/yfinance/fetch-stock-close-price".into()],
            existing_path: None, merge_from: Vec::new(),
        ...
        assert!(r.memories_for_entity("u", "tools/yfinance/fetch-stock-close-price")
            .await.unwrap().iter().all(|held| held.id != memory));
        assert!(r.memories_for_entity("u", &write.path).await.unwrap()
            .iter().any(|held| held.id == memory));
        let library_links = r.links_from_entity("u", "tools/yfinance").await.unwrap();
        assert!(library_links.iter().any(|link| link.to_entity_path == write.path));
        let stale_links = r.links_from_entity("u", "tools/yfinance/fetch-stock-close-price")
            .await.unwrap();
        assert!(!stale_links.iter().any(|link| link.to_entity_path == write.path));
        let effective = crate::db::repo::pkm::PkmConsolidationStore::new(std::sync::Arc::new(r.clone()))
            .scoped(&checkpoint.consolidation_id, "u");
        let fetch = effective.entity_by_path("tools/yfinance/fetch-stock-close-price")
            .await.unwrap().unwrap();
        assert_eq!(fetch.path, "tools/yfinance/fetch-stock-close-price",
    ...
    async fn entity_identity_merge_and_checkpoint_transition_commit_together() {
        ...
        let effective = PkmConsolidationStore::new(committed.clone())
            .scoped("identity-run", "u");
        let contribution = |name: &str, memory_id: &str| PendingEntityContribution {
            name: name.into(), description: "The same person.".into(),
        ...
        let mut canonical = KnowledgeConsolidationEntity::pending(
            "identity-run", "u", "people/full-name", EntityCategory::Concept,
        ...
        let mut losing = KnowledgeConsolidationEntity::pending(
            "identity-run", "u", "people/short-name", EntityCategory::Concept,
        ...
        canonical.aliases.extend(["Short Name".to_string(), "Shared Alias".to_string()]);
        ...
        state.revision = 1;
        let checkpoint = KnowledgeConsolidationRecord {
            id: new_id(), consolidation_id: "identity-run".into(), user_id: "u".into(),
            state: ConsolidationStageState::Classify(state), stats: Default::default(),
            attempts: 0, restart_count: 0, failure: None,
            next_attempt_at: Utc::now(), updated_at: Utc::now(),
        };

        committed.commit_entity_identity_merge(
            &canonical, &losing, &checkpoint,
        ).await.unwrap();

        let stored = effective.working_entity("people/full-name").await.unwrap().unwrap();
        assert!(stored.aliases.contains("Short Name"));
        assert!(stored.aliases.contains("Shared Alias"));
        assert!(stored.source_memory_ids.contains(&"memory-short".to_string()));
        assert_eq!(stored.checkpoint_revision, 1);
        let redirect = effective.working_entity("people/short-name").await.unwrap().unwrap();
        ...
        assert!(next_iteration_candidates.iter()
            .any(|entity| entity.path == "people/full-name"));
        assert!(!next_iteration_candidates.iter()
            .any(|entity| entity.path == "people/short-name"));

        let saved = committed.latest_consolidation_record("u").await.unwrap().unwrap();
