    ...
    async fn ingest_window_commits_as_one_unit() {
        ...
        let batch = |content: &str| IngestBatch {
            entities: vec![PendingEntity {
                ...
                attribute_evidence: attribute_evidence("svc/pg", &serde_json::json!({ "port": "5432" })),
        ...
        assert!(r.memories_for_entity("u", "svc/pg").await.unwrap().is_empty());
    ...
    async fn extract_commit_rolls_back_when_memory_insert_fails() {
        ...
        .await
        .unwrap();
        r.remember("u", "short-chat", "unvalidated source").await.unwrap();
        ...
        let result = r.commit_extract_patch_with_checkpoint(
            "u",
            &batch,
            &[("chat-2".into(), Utc::now())],
        ...
        assert!(result.is_err(), "a failed CREATE must fail the extraction commit");
        assert!(
    ...
    async fn ingest_commits_memory_watermark_checkpoint_and_working_entity_together() {
        ...
        let batch = IngestBatch {
            entities: vec![PendingEntity {
                path: "services/postgres".into(), name: "Postgres".into(),
                description: "relational database".into(), aliases: vec!["PG".into()],
                identity_evidence: Vec::new(),
                attributes: serde_json::json!({"port": "5432"}),
                attribute_evidence: attribute_evidence(
                    "services/postgres", &serde_json::json!({"port": "5432"}),
                ),
            }],
            entity_updates: Vec::new(),
            memories: vec![PendingMemory {
                id: new_id(), kind: MemoryKind::Fact,
                evidence: human_evidence("Postgres listens on 5432", "services/postgres"),
                episode: None, content: "Postgres listens on 5432".into(),
                paths: vec!["services/postgres".into()],
            }],
            playbook_candidates: Vec::new(),
            grounding_corrections: 0,
        ...
        let record = KnowledgeConsolidationRecord {
            ...
            state: ConsolidationStageState::Ingest(extract), stats: Default::default(),
            attempts: 0, restart_count: 0, failure: None,
            next_attempt_at: Utc::now(), updated_at: Utc::now(),
        };
        committed.commit_extract_patch_with_checkpoint(
            "u", &batch, &[("chat-1".to_string(), Utc::now())], &[], &record,
        ).await.unwrap();

        assert!(committed.entity_by_path("u", "services/postgres").await.unwrap().is_none());
        assert_eq!(committed.list_all_memories("u").await.unwrap().len(), 1);
        assert!(committed.consolidation_watermark("chat-1").await.unwrap().is_some());
        let effective = PkmConsolidationStore::new(committed.clone())
            .scoped(&record.consolidation_id, "u");
        let entity = effective.entity_by_path("services/postgres").await.unwrap().unwrap();
        assert_eq!(entity.name, "Postgres");
        assert!(entity.attributes.as_object().is_some_and(|attributes| attributes.is_empty()),
    ...
    async fn ingest_does_not_materialize_a_missing_update_only_entity() {
        ...
        let batch = IngestBatch {
            entities: vec![PendingEntity {
                path: "services/retailer".into(), name: "Retailer".into(),
                description: "online shopping service".into(), aliases: Vec::new(),
                identity_evidence: Vec::new(), attributes: serde_json::json!({}),
            ...
            memories: vec![PendingMemory {
                ...
                evidence: human_evidence(
                    "The retailer processed the return.", "services/retailer",
                ),
                episode: None, content: "The retailer processed the return.".into(),
                paths: vec!["services/retailer".into(), "organizations/retailer".into()],
            }],
            playbook_candidates: Vec::new(), grounding_corrections: 0,
            grounding_items_dropped: 0,
            recall_result_lookups: 0,
            ..Default::default()
        };
        ...
        let effective = PkmConsolidationStore::new(committed.clone())
            .scoped(&record.consolidation_id, "u");
        assert!(effective.working_entity("services/retailer").await.unwrap().is_some());
        assert!(
            effective.working_entity("organizations/retailer").await.unwrap().is_none(),
    ...
    async fn failed_window_leaves_nothing_behind() {
        ...
        assert_eq!(counts.memories_added, 1, "the pageless memory remains durable");
        assert_eq!(counts.entities_created, 0, "extraction never creates entities");
    }
    ...
    async fn stale_external_extraction_cannot_replace_current_memories() {
        ...
        r.upsert_external_page("u", note, "current body", "current-rev")
            .await
            .unwrap();
        let old_memory = r.create_sourced_memory(
            "u",
            MemoryKind::Fact,
        ...
        let applied = r.commit_external_extract_patch_with_checkpoint(
            "u", note, "stale-rev", &batch, &checkpoint,
        ).await.unwrap();
