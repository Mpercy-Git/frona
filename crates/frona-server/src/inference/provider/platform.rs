...
use crate::core::config::{AdapterId, ApiSurface, ModelProviderConfig, ProviderModel};
use crate::inference::credential::store::CredentialMethod;

use crate::inference::error::InferenceError;
use crate::inference::protocol::hooks;
use crate::inference::provider::{
    CredentialValidationError, InferenceCounter, ModelConfig, ModelProvider, OpenAiProvider,
    RigProvider,
};
...
macro_rules! build_listable_key_client {
    ...
    ($name:expr, $config:expr, $endpoint:expr, $module:ident, $counter:expr, hook = $hook:expr) => {{
        let key = require_api_key($name, $config)?;
        let mut builder = $module::Client::builder()
            .http_client(crate::inference::protocol::http::WireClient::default())
            .api_key(&key);
