...
```
memory_search(query | queries=[…]) → up to 8 ranked pages per query; each carries an absolute file path
read(path | paths=[…])             → open page files (prose body + frontmatter attributes/links + ## History)
