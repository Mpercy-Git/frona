...
- **MCP with bridge mode:** install [Model Context Protocol](https://modelcontextprotocol.io) servers from the public registry in a click. Bridge mode advertises a single `mcpctl` CLI to the LLM instead of every MCP tool individually, saving thousands of tokens per turn on agents with many servers connected. The bridge needs the shell tool to run `mcpctl`, so an agent without one falls back to plain per-tool definitions rather than being left with no way to reach a connected server
