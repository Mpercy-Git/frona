...
- **A failed MCP install now says what failed, and stops calling itself installed.** The server row is written before the package is fetched, so a warm-up that resolved nothing left a server sitting at `installed` that never was: pressing Start re-ran the same doomed fetch and answered with npm's transcript - registry chatter, an upgrade nag, and a path to a debug log inside the sandbox, with the one fact that mattered ("that package has no published versions") nowhere in it. npm's and uv's own error codes are now read and translated, so a missing name, a missing version, an unpublished package, a private registry and an unreachable one each read differently; the server is marked `failed` with that reason kept beside it, so its page still explains itself a week later instead of sending the reader to the logs; and the reason is cleared by the reinstall that works. Each failure is explained from what that attempt wrote, not from what an earlier one left in the same appended log, and a failed install keeps the sandbox policy it was given so `Update` can retry in place
