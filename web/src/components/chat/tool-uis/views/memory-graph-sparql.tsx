...
/**
 * Parse `GraphQueryOutput` as the backend serializes it
 * (crates/frona-server/src/memory/pkm/ontology/graph_query.rs). Keep in lockstep:
 *
 *   { "kind": "boolean", "value": true }
 *   { "kind": "solutions", "columns": ["person"],
 *     "rows": [{ "person": "people/mina" }], "truncated": false }
 *
 * A row omits the key for a variable the solution left unbound, so a missing
 * cell is data rather than a parse failure. Returns null on anything else so
 * the caller can hand off to the generic view.
 */
...
function decodeResult(result: unknown): { parsed: SparqlResult | null; raw: string } {
  if (result === undefined) return { parsed: null, raw: "" };
  if (typeof result !== "string") {
    return { parsed: parseSparqlResult(result), raw: JSON.stringify(result, null, 2) };
  }
  try {
...
function resultSubtitle(result: SparqlResult | null): string | null {
  ...
  const noun = result.rows.length === 1 && !result.truncated ? "row" : "rows";
  return `${count} ${noun}`;
}
        ...
        <tbody>
          {result.rows.map((row, rowIndex) => (
            <tr
              ...
              className="even:bg-surface-tertiary/30"
            >
              {result.columns.map((column) => (
                <td
                  key={column}
...
export const MemoryGraphSparqlView: ToolView = ({
  ...
  const error =
    status?.type === "incomplete" ? (status as { error?: unknown }).error : undefined;
  const errorText =
    error == null ? null : typeof error === "string" ? error : JSON.stringify(error);

  return (
    <ToolRow status={status} expandable={query.length > 0 || raw.length > 0}>
      ...
      <ToolRow.Body isExpanded={isExpanded} unstyled>
        <div className="flex flex-col gap-3">
          {query && <CodeBlock code={query} language="sparql" wrap />}
